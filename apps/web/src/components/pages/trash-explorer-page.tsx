    ...
    try {
      ...
      if (result.success) {
        if (result.hasConflict) {
          toast.success(`Restored as "${result.finalName}" (due to name conflict)`);
        } else {
            ...
            <div className='grid [grid-template-columns:repeat(auto-fill,minmax(200px,1fr))] gap-4'>
              {folders.map(folder => (
                <ExplorerItem
                  ...
                  view='grid'
                  isTrashView
                  onRestore={() => handleRestoreItem(folder.id, true, folder.name)}
                />
              ))}
              {files.map(file => (
                <ExplorerItem
                  ...
                  view='grid'
                  isTrashView
                  onRestore={() => handleRestoreItem(file.id, false, file.name)}
                />
              ))}
            </div>
          </div>
