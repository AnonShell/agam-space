...
export function ExplorerItem({
  ...
  const fileSizeText =
    !entry.isFolder && entry.size ? formatBytes(entry.size) : null;
  ...
  return (
    <>
      <ContextMenu  onOpenChange={(open) => {
        ...
        <ContextMenuContent
          className="w-48 bg-white dark:bg-zinc-900 text-black dark:text-white border shadow-md rounded-md z-50">
          {isTrashView ? (
            <>
            ...
            <>
              <ContextMenuItem onClick={() => href && router.push(href)}>Open</ContextMenuItem>
              {!entry.isFolder && (
              ...
              {entry.isFolder && (
                <ContextMenuItem onClick={() => onRecomputeSize(entry)}>Refresh size</ContextMenuItem>
              )}
              <ContextMenuItem
